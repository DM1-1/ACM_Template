...
int sz[N], hson[N], rnk[N], col[N], L[N], R[N], tot_dfn; //L, R是dfn序
int ans[N], cnt[N], tot_color;
...
{
    cnt[col[x]] ++;
    if(cnt[col[x]] > Max_cnt) 
    {
        Max_cnt = cnt[col[x]];
        sum = col[x];
    }
    else if(cnt[col[x]] == Max_cnt)
        sum += col[x];
}
...
int get(int x) //离散化查询
{
    return lower_bound(all(ind), x) - ind.begin();
}

void pushup(int p)
{
    seg[p].cnt = seg[seg[p].ls].cnt + seg[seg[p].rs].cnt;
}

int build(int l, int r) //初始化空树，这里可以不用
{
    int p = ++ tot;
    if(l == r) {
        seg[p].cnt = 0;
...
int modify(int old, int l, int r, int pos)
{
    int p = ++ tot;
    seg[p] = seg[old]; //动态开出一个新的节点，跟之前的版本一样，但是接下来的左右儿子一定有一个会发生改变。
    if(l == r) {
        seg[p].cnt ++;
        return p;
    }
    int mid = (l + r) >> 1;
    if(pos <= mid)
        seg[p].ls = modify(seg[old].ls, l, mid, pos);
    else
        seg[p].rs = modify(seg[old].rs, mid + 1, r, pos);
    pushup(p); //要记得合并信息。
    return p;
}

int query(int q, int p, int l, int r, int k) //在版本q和p上找
{
    if(l == r)  
        return l;
    int mid = (l + r) >> 1;
    int sum = seg[seg[q].ls].cnt - seg[seg[p].ls].cnt;
    if(k <= sum) //继续找第k小
        return query(seg[q].ls, seg[p].ls, l, mid, k);
    else //找第k - sum小
        return query(seg[q].rs, seg[p].rs, mid + 1, r, k - sum);
}
...
int main()
{
    ...
    //rt[0] = build(1, n); 本题不需要初始化空树
    for(int i = 1; i <= n; i ++)
        rt[i] = modify(rt[i - 1], 0, len, get(a[i]));
    
    while(m --)
    {
        int l, r, k; //通过区间减法可知，[1, r] - [1, l - 1]就是[l, r]
        cin >> l >> r >> k;
        cout << ind[query(rt[r], rt[l - 1], 0, len, k)] << '\n'; 
    }
}
```
...
int query(int v, int l, int r, int ql, int qr)
{
    ...
    int s = 0;
    if(ql <= mid)
        s += query(seg[v].ls, l, mid, ql, qr);
    if(mid < qr)
        s += query(seg[v].rs, mid + 1, r, ql, qr);
    return s;
}
...
int main()
{
    ios::sync_with_stdio(false);cin.tie(0);
    cin >> n;
    vector<int> last(1000001, 0); //last[x]:x上次出现的下标
    for(int i = 1; i <= n; i ++)
    {
        int x;
        cin >> x;
        rt[i] = modify(rt[i - 1], 1, n, i, 1); //表示[i, ]贡献+1
        if(last[x] != 0)
            rt[i] = modify(rt[i], 1, n, last[x], -1); //去掉之前的贡献
        last[x] = i;
    }
...
void modfiy(int k, int l, int r, int pos, int v)
{
    if(l == r) {
        T1[k] += v;
        return;
    ...
    int mid = (l + r) >> 1;
    if(pos <= mid)
        modify(k << 1, l, mid, pos, v);
    else
        modify(k << 1 | 1, mid + 1, r, pos, v);
...
int main()
{
    ...
    for(int i = 1; i <= m; i ++)
    {
        while(cnt + 1 <= q[i].r) 
        {
            cnt ++;
            if(last.count(a[cnt]))    
                modify(1, 1, n, last[a[cnt]], -a[cnt]);
            last[a[cnt]] = cnt;
            modify(1, 1, n, cnt, a[cnt]);
        }
...
int main()
{
    ...
    for(int i = 1; i <= n; i ++)
    {
        ...
        scanf("%d%d", &x, &y);
        vx.push_back(x);
        events.push_back({y, 0, x, 0});
    }
    for(int i = 1; i <= q; i ++)
    {
        int x1, x2, y1, y2;
        scanf("%d%d%d%d", &x1, &x2, &y1, &y2);
        //y坐标 事件类型（优先级） x坐标 第i次查询
        events.push_back({y2, 2, x2, i});
        events.push_back({y2, 1, x1 - 1, i});
        events.push_back({y1 - 1, 1, x2, i});
        events.push_back({y1 - 1, 2, x1 - 1, i});
    }
    ...
    for(auto evt : events) 
    {
        if(evt[1] == 0) 
        {
            int y = lower_bound(vx.begin(), vx.end(), evt[2]) - vx.begin() + 1; //因为树状数组下标从1开始
            add(y, 1);
        }
        else 
        {
...
int anc[N][21];
int dep[N];
vector<PII> g[N]; //first->val second->edge
ll val[N];
...
{
    return dep[x] - 2 * dep[LCA(x, y)] + dep[y];
}

void init()
{
    for(int i = 1; i <= n; i ++)
        g[i].clear(), dep[i] = 0, val[i] = 0;
    for(int i = 1; i <= n; i ++)
        for(int j = 0; j < 21; j ++)
            anc[i][j] = 0;
    //建图
...
NEGATE a b 将 a 到 b 的路径上每条边权值取相反数
QUERY a b 查询 a 到 b 的路径上最大权值
DONE 表示操作结束
...
int nex[N << 1], e[N << 1], val[N << 1], num[N << 1], h[N], idx;
...
int hson[N], deep[N], sz[N], father[N];
int dfn[N], rnk[N], top[N], tot = 0;
...
struct Seg_tree {
    ...
    void build(int k, int l, int r)
    {
        seg[k].l = l, seg[k].r = r;
        if(l == r) {
            seg[k].max_v = seg[k].min_v = pv[rnk[l]];
            return;
        }
        int mid = (l + r) >> 1; 
        build(k << 1, l, mid);
    ...
    void pushup(int k)
    {
        seg[k].max_v = max(seg[k << 1].max_v, seg[k << 1 | 1].max_v);
        seg[k].min_v = min(seg[k << 1].min_v, seg[k << 1 | 1].min_v);
    }
    void update(int k)
    {
        int &mx = seg[k].max_v, &mn = seg[k].min_v;
        swap(mx, mn);
        mx = -mx;
        mn = -mn;
        seg[k].lazy ^= 1;
    ...
    void modify1(int k, int ql, int qr) //取反
    {
        int l = seg[k].l, r = seg[k].r;
        if(ql <= l && r <= qr) {
            update(k);
    ...
    void modify2(int k, int pos, int v)
    {
        int l = seg[k].l, r = seg[k].r;
        if(l == r) {
            seg[k].max_v = v;
    ...
    int query(int k, int ql, int qr)
    {
        ...
        int res = -2e9;
        if(ql <= mid)
            res = max(res, query(k << 1, ql, qr));
        if(mid < qr)
            res = max(res, query(k << 1 | 1, ql, qr));
        return res;
    }
};
Seg_tree T1;
...
void tree_find1(int x, int y)
{
    while(top[x] != top[y])
    {
        if(deep[top[x]] < deep[top[y]])
            swap(x, y);
        T1.modify1(1, dfn[top[x]], dfn[x]);
        x = father[top[x]];
    }
    if(deep[x] > deep[y])
        swap(x, y);
    if(x != y)  
        T1.modify1(1, dfn[x] + 1, dfn[y]);
}
int tree_find2(int x, int y)
{
    int s = -2e9;
    while(top[x] != top[y])
    {
        if(deep[top[x]] < deep[top[y]])
            swap(x, y);
        s = max(s, T1.query(1, dfn[top[x]], dfn[x]));
        x = father[top[x]];
    }
    if(deep[x] > deep[y])
        swap(x, y);
    if(x != y)  
        s = max(s, T1.query(1, dfn[x] + 1, dfn[y]));
    return s;
}   
...
int main() {
    ...
    getHash(1, 0);
    printf("%llu", trees.size());
}
```

### 无根树哈希（换根）
给 $n$ 个树，求与每个树同构的数的最小编号。
```cpp
typedef unsigned long long ull;
...
那如果我想查询区间 $[l, r]$ 的数字与x异或后最大的那个数，怎么办？求异或的这个问题，是通过字典树来解决的，所以区间询问这个问题，就是典中典的可持久化数据结构应用问题了。如果你**真正**的学会了可持久化线段树的话，那么YY一下也就能改出可持久化字典树的代码了。
...
// 树的重心
int n;
int mn = 1e9, res; //图中删去重心后最大联通块的最小值，重心节点编号
vector<int> g[N];
int sz[N], sum[N]; //sz:子树大小 sum:设u为重心，取其子树最大值

void dfs(int u, int fa)
{
    sz[u] = 1;
    sum[u] = 0; //设u为重心，取其子树最大值
    ...
    sum[u] = max(sum[u], n - sz[u]); //还有一个来自父节点的子树
    if(sum[u] < mn) mn = sum[u], res = u;
} 
...
```c++
int n, c, d[N]; //c是用来求端点的 d[i] = 结点i距离我们所指定的根节点的距离
vector<int> g[N];
int father[N];
...
int main() 
{
    ...
    dfs(1, -1);
    d[c] = 0, dfs(c, -1); //以直径一次端点为根，寻找另一个端点
    printf("%d\n", d[c]);
}
...
//树形dp法
int n, d = 0; //树的直径的长度
int d1[N], d2[N]; //最长距离，次长距离
vector<int> g[N];

void dfs(int u, int fa)
{
    d1[u] = d2[u] = 0;  //init 且叶子结点的最远距离和次远距离均为0
    for(auto v : g[u])
    {
        if(v == fa) continue;
        dfs(v, u);
    ...
    d = max(d, d1[u] + d2[u]); //对每个结点更新一次树的直径长度
}
...
快速求 
$$
M(n) = \sum_{i = 1}^n\mu(i), (n\le 10^{11})
$$
...
//FFT 多项 式乘 法
//n和m 表 示 两 个 多 项 式 的 次 数
//a[i].x和b[i].x 表示第 i项
// 结果 保 存在 a[i].x中 ， 注 意 是 浮 点 数 ， 需 要 取 整 输 出
const int N = 1<<20;
// #define double long double
const double pi = acos(-1);
typedef vector <ll> poly;
// 根 据 精 度 判 断 double 还是 long double
//long double 可 能 会 导 致 速 度 变 慢
//#define double long double
struct com
{// 复数
...
// #define double long double
// #define ll __int128
// 精 度 不 够 的 时 候 要 开 int128和long double
// 空间要开 4倍 多项 式 长度 2n， 扩域 最 多到 4n
const int N = 1<<20;
typedef vector <ll> poly;
...
poly operator*(poly &a, poly &b)
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        B[i] = Complex(b[i]>>L, b[i]&MASK);// 拆成 两 部分
    }
    FFT(A, n); FFT(B, n);
    for(int i=0; i<n; i++)
    {
        int j = (n-i)%n;
        //conj 表 示 返 回 一 个 complex 的共轭
        // 压缩值域 ， 前 一 半 和 后 一 半
...
​           *•* 阶 (即幂的最小循环节): $a$ 在模  $p$  意义下的阶是：最小的整数  $k$，使得 $a^ k  ≡ 1(mod\ p)$，记作 $δp(a)$

性质 1. : 设 $p ≥ 1, gcd(a, p) = 1$，若 $a^n ≡ 1(mod\ p), n > 0$，则 $δp(a)|n$

性质 2. : 设 $δp(a)|ϕ(p)$

推论 1. : 若 $p, q$ 均为奇质数，且 $q|(a^p− 1)$ 则有 $q|(a − 1) $或 $2p|(q − 1)$

推论 2. ：原根 (满足 $δp(g) = ϕ(p)$ 的 $g$，也即 $g^ϕ(p) ≡ 1(mod\ p))$
